...
def fnGetDateFormatStr():
    return "%m/%d/%Y"

def fnGetISODateFormatStr():
    return "%Y-%m-%d"

def fnInvalidDate():
    return dt(year=1,month=1,day=1)

def fnGetDateStr(d, pfnFormat = fnGetDateFormatStr):
    if type(d) == str:
        return d
    elif not pd.isnull(d) and d != fnInvalidDate():
        return d.strftime(pfnFormat())
    else:
        return ""

def fnGetDateFromStr(dtStr, fnFormat = fnGetDateFormatStr):
    ret = fnInvalidDate()
    
    try:
        if dtStr != "":
            ret =  dt.strptime(dtStr, fnFormat())
    except:
        print("Unable to parse date: ", dtStr)
        
    return ret


def fnPrintJobResults(dfSearchResults, columns=["title", "company", "city", "employment_type", "work_model"]):
    with pd.option_context('display.max_colwidth', None):
        display(dfSearchResults[columns])



def fnParsePosted(str, date):
    try:
        ...
        if len(numbers) == 1 and len(units) == 1:
            ...
            # Nearest day is sufficient. For more precise info look at linkedin's posted date.
                ...
                print("Unexpected unit: ", units[0])
                date = None
                
    except Exception as e:
        print("Unable to parse posted date/time: ", e)
        date = None
    dateStr = ""
    if date is not None:
        dateStr = fnGetDateStr(date)
        
    return dateStr

def fnParseAppliedCount(str):
    appliedCount = -1
    ...
    try:
        ...
        if len(numbers) == 1:
            ...
            print("Error: applied count can't be parsed")
            
    except Exception as e:
        print("Error: applied count can't be parsed due to exception")
        print(e)

    return appliedCount

def fnParseLocation(geolocator, str, job):
    success = False
    ...
    try:
        ...
        if str.find(",") != -1:
            ...
            if location is not None:
                address = geolocator.reverse(location.raw["lat"] + ", " + location.raw["lon"]).raw["address"]
        ...
        print("Error: exception while trying to parse location.")
        print(e)
    ...
    if not success:
        print(f"Warning: unable to geocode {str}. Falling back to simple tokenization.")
        placename = ""
        state     = ""
        ...
        try:
            ...
            print(f"Error: unable to apply simple tokenization logic to location: {str}")
            print(e)
...
def fnParseDetails(geolocator, lsDetails, job):
    ...
    if len(lsDetails) > 0:
        ...
        matchLocation = fnParseLocation(geolocator, lsDetails[0], job)
        ...
        for i in range(1, len(lsDetails)):
            ...
            if re.match("^\s*·\s*", detail) is None:    
                ...
                # Employment time: 
                    ...
                    print(f"Warning: unexpected detail found: {detail}")
    else:
